...
#ifdef SN_TX_CAPABLE
static uint8_t sn_tx_buf[(SN_MAX_MESSAGE_LEN * 2) + SN_PREAMBLE_LEN] 
...
static uint8_t sn_tx_bit = 0;

// Sample number for the transmitter. Runs 0 to 7 during one bit interval
static uint8_t sn_tx_sample = 0;
...
void sn_setup(void)
{
	...
	#elif ((SN_SPEED*SN_RX_SAMPLES_PER_BIT) < 32000)
...
#define _O_VAL_  ((F_CPU/SN_TIMER_PRESCALE) / (SN_SPEED * SN_RX_SAMPLES_PER_BIT))
#if (_O_VAL_ > 255)
...
uint8_t sn_send(uint8_t* buf, uint8_t len)
{
	...
	//DBG_STRP_U8H_NL("\tsn_send() len=",len);

    if (len > SN_MAX_PAYLOAD)
	return false;

	//DBG_STRP_U8H_NL("\t\twaiting...sn_tx_enabled=",sn_tx_enabled);
    // Wait for transmitter to become available
    sn_wait_tx();
	//DBG_STRP_U8H_NL("\t\tdone...sn_tx_enabled=",sn_tx_enabled);
	...
	//DBG_STRP_U8H_NL("\t\tstarting tx.sn_tx_len=",sn_tx_len);
	//DBG_STRP_U8H_NL("\t\tsn_tx_index=",sn_tx_index);
	//DBG_STRP_U8H_NL("\t\tsn_tx_bit=",sn_tx_bit);
    sn_tx_start();
	//DBG_STRP_U8H_NL("\t\ttx started. sn_tx_enabled=",sn_tx_enabled);
...
ISR(TIM0_COMPA_vect)
{
...
#endif
#ifdef SN_RX_CAPABLE
    sn_rx_sample = (PIN(RX_PORT) & RX_MASK) >> RX_BIT;
#endif
#ifdef USE_MILLIS
    ...
    if (sn_tx_enabled && sn_tx_sample++ == 0)
    {
        // Send next bit
		// Symbols are sent LSB first
		// Finished sending the whole message? (after waiting one bit period 
		// since the last bit)
        if (sn_tx_index >= sn_tx_len)
		{
			sn_tx_stop();
			sn_tx_msg_count++;
		}
		...
		{
			if (sn_tx_buf[sn_tx_index] & (1 << sn_tx_bit++))
				PORT(TX_PORT) |= TX_MASK;
			else
				PORT(TX_PORT) &= ~TX_MASK;
			if (sn_tx_bit >= 6)
...
#endif
#ifdef SN_RX_CAPABLE
    if (sn_rx_enabled && !sn_tx_enabled) sn_pll();
#endif
#ifdef LED_PORT
	//PORT(LED_PORT) &= ~LED_MASK; // turn off
#endif
